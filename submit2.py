...
def _3187():
    ...
    check = [[-1] * C for _ in range(R)] # 방문한 좌표는 1로 갱신
    ...
    def DFS(cur_x, cur_y):
        ...
        for i in range(4): # 상하좌우로 이동해서 다음 좌표를 설정
            ...
            if (0 <= next_x <= R-1) and (0<= next_y <= C-1): # 좌표는 최소 (0, 0) 최대 (R-1, C-1)
                if (ranch[next_x][next_y] not in '.#') and (check[next_x][next_y] == -1): # 이동한 좌표 역시 양이나 늑대가 있으면서 방문한 적이 없으면
                    DFS(next_x, next_y) # 조건을 만족하는 동안은 계속해서 탐색

    # 총 RxC개의 모든 좌표에 대해 검사
    for cur_x in range(R):
        for cur_y in range(C):
            if ranch[cur_x][cur_y] not in '.#':
                if check[cur_x][cur_y] == -1: # 빈 공간이거나 울타리면 다음 좌표로 넘어감양이나 늑대가 있으면서 방문한 적이 없으면
